...
from tensorflow.keras.applications import ResNet50, ResNet101, ResNet152  # Different ResNet variants
...
def load_and_preprocess_image(image_path, target_size=(224, 224)):
    """
    Load an image, resize it, and preprocess using ResNet's preprocess_input function.
    """
    img = image.load_img(image_path, target_size=target_size)
    img_array = image.img_to_array(img)
    # Use ResNet's specific preprocessing
...
# Choose ResNet model variant (ResNet50, ResNet101, or ResNet152)
base_model = ResNet50(weights='imagenet', include_top=False, input_shape=(224, 224, 3))

# Freeze early layers
base_model.trainable = True
for layer in base_model.layers[:100]:  # Adjust the number of frozen layers as needed
    layer.trainable = False

x = base_model.output
x = GlobalAveragePooling2D()(x)
x = Dense(1024, activation='relu')(x)
x = Dropout(0.5)(x)  # Increased dropout for ResNet
...
# Save the Model
model.save('CountryClassifier_ResNet50_10_epochs.keras')
...
def plot_class_accuracy(y_true, y_pred, class_names, top_n=10):
    ...
    plt.tight_layout()
    plt.savefig('ResNet50_top_classes.png')
    plt.show()
